...
- [x] **Decide: fold into the existing `scan-tools/` image, or a new subdirectory?** Decision: folded in, same reasoning as `testssl.sh` — one-shot, single-target CLI scan, not a daemon.
- [x] Confirm the package name/install path for `nikto` in Kali's apt repos and add it to `scan-tools/Dockerfile` — `nikto` package confirmed available, added alongside `testssl.sh` in the same `apt-get install` line.
- [x] **Confirm capability requirements empirically, don't assume**: confirmed — `nikto` needs neither `--cap-add=NET_RAW`/`NET_ADMIN` (an HTTP client, not raw-socket tooling, same reasoning as `testssl.sh`) **nor `--network host`** (outbound HTTP requests get NAT'd through Docker's default bridge network fine; `--network host` only matters for tools operating at Layer 2 like `arp-scan`, or doing raw-socket scan types like `nmap`). Both predictions verified by running without the flags before assuming they weren't needed.
- [x] Decide accepted target format(s) — `nikto -h` accepts a full URL with scheme directly (confirmed via `nikto -H`'s own docs: "`-host+ Target host/URL`"), so `$TARGET` is passed straight through unmodified, same as every other script — caller decides `http://` vs `https://` by what they type, no separate `-ssl` flag needed.
- [x] Check whether `nikto` has a native "write results to file" flag — yes: `-output <file>` plus `-Format txt` (or other formats). Hit a real bug working this out: nikto does NOT strip/detect an existing extension on the `-output` path — it unconditionally appends the `-Format`-determined extension regardless, so a filename that already ended in `.txt` came out `.txt.txt` doubled. Fix: build `$OUTFILE` for this script *without* a `.txt` suffix, and pass `-Format txt` explicitly so nikto appends exactly one clean extension. Verified with a `-maxtime 5s`-truncated test run before applying to the real script.
- [x] Same conventions as every other `scan-tools` script: `--help`, required target, timestamped output into `~/kali-data`
- [x] Add the new script to `ci.yml`'s `shellcheck` glob — confirmed already covered by the existing `scan-tools/scripts/*.sh` glob, no change needed (same as `testssl.sh`).
- [x] Test against the real, already-known target: the router admin panel (`192.168.1.254`) — confirmed and extended `testssl.sh`'s findings from a different layer: session cookie missing the `Secure` flag, missing `referrer-policy`/`permissions-policy`/`content-security-policy`/`X-Content-Type-Options` headers, and independently re-confirmed the hostname/cert mismatch from the HTTP layer.
